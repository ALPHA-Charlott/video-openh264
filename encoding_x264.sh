...
${ffcmd} -hide_banner -y -threads 8 -i $VIDEO_IN \
    -preset $PRESET_P -keyint_min $GOP_SIZE -g $GOP_SIZE -sc_threshold 0 -r $FPS -c:v libx264 -pix_fmt yuv420p \
    -map v:0 -s:0 $V_SIZE_1 -b:v:0 144k -r:v:0 15 -maxrate:0 180k -bufsize:0 320k \
    -map v:0 -s:1 $V_SIZE_2 -b:v:1 180k -maxrate:1 210k -bufsize:1 360k \
    -map v:0 -s:2 $V_SIZE_3 -b:v:2 735k -maxrate:2 780k -bufsize:2 1M \
    -map v:0 -s:3 $V_SIZE_4 -b:v:3 900k -maxrate:3 1.1M -bufsize:2 1.5M \
    -map v:0 -s:4 $V_SIZE_5 -b:v:4 3M   -maxrate:4 3.2M -bufsize:3 5M \
    -map v:0 -s:5 $V_SIZE_6 -b:v:5 6M  -maxrate:4 6.6M -bufsize:3 10.6M \
    -map a:0 -map a:0 -map a:0 -map a:0 -map a:0 -map a:0 \
    -c:a libfdk_aac -ac 2 -b:a 128k -r:a 44100 \
    -f hls -hls_time $HLS_TIME -hls_playlist_type vod -hls_flags independent_segments \
    -master_pl_name $VIDEO_OUT.m3u8 \
    -hls_segment_filename HLS/${title}_stream_%v/s%06d.ts \
    -strftime_mkdir 1 \
    -var_stream_map "v:0,a:0 v:1,a:1 v:2,a:2 v:3,a:3 v:4,a:4 v:5,a:5" ${title}_stream_%v.m3u8
